package com.example.idan.plusplus.V2.Services.Retrofit2Services.Epgs.Israel.Live.ChannellMovixFictionService;
...
public class ChannellMovixFiction extends BaseAbstractService {

    private GridItem localGridItem;

    public ChannellMovixFiction(SharedPreferences sharedPreferences,OnAsyncTaskLoadCompletes completeCallback) {
        super(sharedPreferences,completeCallback);
    }



    public void getLiveChannellMovixFiction(FragmentActivity fragmentActivity, int rootFragment, GridItem gridItem, OnAsyncTaskLoadCompletes<GridItem> callback) {
        freshStartChannell();
        ...
        headers.put("User-Agent","MovixFiction-KODI");
      //  headers.put("Accept,"text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8");
        ...
        setHeaders(headers);
              ...
              getHtmlFromUrl("https://movix.live/genre/Fiction/",false)
    ...
    private ObservableSource<String> getLiveChannellNext(String t) {
        ...
        //    na=name.replace('Home','ראשי').replace('Fiction','פעולה').replace('Comedy','קומדיה').replace('Crime','פשע').replace('Drama','דרמה').replace('Mystery','מיסטורין').replace('Thriller','מותחן')
