package com.example.idan.plusplus.V2.Services.Retrofit2Services.Epgs.Israel.Live.ChannellMovixMysteryService;
...
public class ChannellMovixMystery extends BaseAbstractService {

    private GridItem localGridItem;

    public ChannellMovixMystery(SharedPreferences sharedPreferences,OnAsyncTaskLoadCompletes completeCallback) {
        super(sharedPreferences,completeCallback);
    }



    public void getLiveChannellMovixMystery(FragmentActivity fragmentActivity, int rootFragment, GridItem gridItem, OnAsyncTaskLoadCompletes<GridItem> callback) {
        freshStartChannell();
        ...
        headers.put("User-Agent","MovixMystery-KODI");
      //  headers.put("Accept,"text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8");
        ...
        setHeaders(headers);
              ...
              getHtmlFromUrl("https://movix.live/genre/Mystery/",false)
    ...
    private ObservableSource<String> getLiveChannellNext(String t) {
        ...
        //    na=name.replace('Home','ראשי').replace('Mystery','פעולה').replace('Comedy','קומדיה').replace('Crime','פשע').replace('Drama','דרמה').replace('Mystery','מיסטורין').replace('Thriller','מותחן')
