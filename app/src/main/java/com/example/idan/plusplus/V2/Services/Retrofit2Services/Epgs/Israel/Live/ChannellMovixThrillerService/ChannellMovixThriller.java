package com.example.idan.plusplus.V2.Services.Retrofit2Services.Epgs.Israel.Live.ChannellMovixThrillerService;
...
public class ChannellMovixThriller extends BaseAbstractService {

    private GridItem localGridItem;

    public ChannellMovixThriller(SharedPreferences sharedPreferences,OnAsyncTaskLoadCompletes completeCallback) {
        super(sharedPreferences,completeCallback);
    }



    public void getLiveChannellMovixThriller(FragmentActivity fragmentActivity, int rootFragment, GridItem gridItem, OnAsyncTaskLoadCompletes<GridItem> callback) {
        freshStartChannell();
        ...
        headers.put("User-Agent","MovixThriller-KODI");
      //  headers.put("Accept,"text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8");
        ...
        setHeaders(headers);
              ...
              getHtmlFromUrl("https://movix.live/genre/Thriller/",false)
    ...
    private ObservableSource<String> getLiveChannellNext(String t) {
        ...
        //    na=name.replace('Home','ראשי').replace('Thriller','פעולה').replace('Comedy','קומדיה').replace('Crime','פשע').replace('Drama','דרמה').replace('Mystery','מיסטורין').replace('Thriller','מותחן')
